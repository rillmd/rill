---
name: eval
description: Quantitatively evaluate the search quality and efficiency of the Rill knowledge base via natural exploration vs Deep Path ground truth (ADR-050). Run monthly. Computes precision, recall, metadata_contribution, token cost. Use when the user wants to verify whether note structure actually works in practice.
---
...
Generate the ground truth set with **Deep Path** (unconstrained exhaustive exploration), then process the same queries with **the AI agent's natural exploration**. If the discovery rate via metadata (metadata_contribution) is high, it proves that note structuring is effective.

While /inspect's structural reachability check (Phase 1.8) deterministically measures "whether metadata paths exist," /eval empirically measures "whether the AI agent can actually reach them."
...
Launch the Deep Path sub-agent for all 20 queries in `eval/queries.yaml`. Execute in parallel batches of 5.
...
### Phase 2: Natural Exploration (7 queries)
...
2. For each query, execute natural exploration via a sub-agent (7 sub-agents in parallel)
...
Aggregate usage information from each sub-agent:
...
- Provide natural exploration sub-agents with Rill structural knowledge (equivalent to CLAUDE.md / AGENTS.md information)
