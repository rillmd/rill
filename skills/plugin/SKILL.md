...
1. Run `rill plugin list` in a shell
...
2. Run `rill plugin install <name>` in a shell (always succeeds)
...
1. Run `rill plugin enable <name>` in a shell
...
2. Run `rill plugin disable <name>` in a shell
...
3. Confirm with the user (via the harness's question primitive)
4. Run `rill plugin uninstall <name>` in a shell
