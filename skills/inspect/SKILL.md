---
name: inspect
description: Diagnose Rill PKM taxonomy health, metadata accuracy, and file integrity. Run weekly. Detects issues and appends mismatched files to knowledge/.refresh-queue for repair by /repair.
---
...
4. Aggregate frontmatter from all knowledge/notes/*.md files. If the file count is large (100+), use a sub-agent for parallel processing:
...
2. Use a sub-agent to Read sample file contents and evaluate on the following **5 axes**:
...
- When processing a large number of files, use a sub-agent for parallelization to conserve context
