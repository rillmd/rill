---
name: onboarding
description: Guide a new user through a 5-8 minute first-time Rill setup — language detection, first journal entry, core concepts (/focus + /distill), an "ask the assistant" prompt, optional morning reminder, and optional Rill app install on macOS. Use when the user runs first-time setup or asks for a tutorial.
gui:
  label: "/onboarding"
...
# Check for an existing user profile
...
- **`knowledge/self/profile.md` exists**: Skip the name question in Phase 1.
...
After the user responds, capture it using the harness's file-write primitive (do **not** use `rill log` — it would expose the CLI to the user):
...
4. Use the harness's scheduling mechanism to schedule `/morning` for that time with the cron expression. **This is an internal tool call — do not surface the implementation name to the user.**
   - On Claude Code: invoke the `schedule` skill or `CronCreate`
   - On other harnesses (e.g., Codex CLI): register a system cron entry or launchd plist that runs `/morning` via the harness's headless mode
...
1. Determine a repo-relative path to name for the user, in priority order. Use the first that exists; if none do, skip to Closing.
   ...
   - `knowledge/self/profile.md` — fallback
...
| Situation | Handling |
|---|---|
| vault marker missing | Warn before Phase 1. Tell the user the vault hasn't been initialized yet — they can either ask the assistant to *"initialize the vault here"* (the assistant will run `rill init`) or run `rill init` themselves from the terminal. Do not lead with the CLI command |
...
| knowledge/self/profile.md already exists | Skip name question. Use existing name in greeting if readable |
...
| GUI hand-off: no briefing / journal / self/profile.md exists | Skip the hand-off step. Use the fallback closing |
...
- Schedule `/morning` only after explicit user confirmation (Claude Code: via `schedule` skill / `CronCreate`; other harnesses: system cron / launchd)
- Always end with a warm closing in `DETECTED_LANG`
- When scaffolding `knowledge/self/` (Phase 1), always create files via `rill mkfile` so timestamps are authoritative — never hardcode them
