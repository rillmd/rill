---
name: solve
description: Execute a Rill task ticket end-to-end via a Plan with embedded breakpoints (Plan-Embedded Breakpoint pattern). Each Plan step declares [Claude] or [User] and whether a breakpoint is required. Resumable across sessions via `## Current Position` in `_task.md`. Use when the user wants to work on or resume a specific task.
gui:
  label: "/solve"
...
> **Tool references in this skill** (`Read`, `Edit`, `Grep`, `Glob`, `WebSearch`, `AskUserQuestion`, `find`, `shell`) describe **intent**, not Claude-specific tool calls. Each harness should map them to its native equivalent — Claude Code uses its built-in tools as named; Codex CLI uses `apply_patch` / `--search` / shell / its own question primitive as appropriate.
>
> The `[Claude]` Plan tag is a literal label denoting "AI agent step" regardless of which harness is executing. Do not rename existing tags; for new tasks the AI side may use either `[Claude]` or `[AI]` interchangeably.

The default is to solve a task in a single ticket. Each step in the Plan declares **who acts** and **whether a breakpoint is required**, and the AI navigates that Plan seamlessly. There is no explicit "AI autonomous mode" → "human-in-the-loop" toggle — the Plan itself dictates when the AI proceeds and when the user is called.
...
| Email / external messaging | Draft only by the AI. Sending is a `[User]` step with `⚠️ Breakpoint` |
...
Current Position is written as the **first Edit of the next Step**, not as the last Edit of the previous Step. If the AI is interrupted, what remains in the file is "the step the AI most recently opened" — which is exactly the right resume point. Do this at the top of every step in Phase 4 Execute.
...
The AI drafts the Plan. **`[Claude]` / `[User]` tags and `⚠️ Breakpoint` markers are required.**
...
| **Action** | External submission / send | `[User]` step with `⚠️ Breakpoint`. The AI drafts to `tasks/{slug}/NNN-draft-*.md`; the user sends |
...
2. Ask for approval via the harness's question primitive:
   ```
   ## Execution Plan
...
for step in Plan.steps:
    ...
    if step.tag == "[Claude]":
        ...
        Tell the user in one line what to do, and confirm completion via the harness's question primitive
    if step.has_breakpoint:
        - Review request → wait via the harness's question primitive for approval / change request
...
- Keep `{topic}` short and identifying — e.g. `tasks/fix-knowledge-axios-investigation-acme-saas-repo/_task.md`
