---
name: repair
description: Batch-update knowledge/notes/ frontmatter (tags, mentions, type) for files queued by /inspect. Reads `knowledge/.refresh-queue` and dispatches parallel sub-agents to repair metadata. Use when the user wants to fix queued metadata issues, typically right after /inspect.
---
...
4. Launch **up to 4 sub-agents in parallel**. If there are more than 4 batches, wait for previous batches to complete before launching the next
...
Launch as background sub-agents (one per batch). On Claude Code, use `run_in_background: true` with `model: "sonnet"` — the Sonnet routing was validated 2026-04-19 in the Tier 1 routing eval (3/3 PASS, 53% cost reduction vs Opus). Haiku was tested but failed (1/3 PASS — catastrophic field hallucination on multi-rule cases including invented tags and corrupted source/created fields), so do not downgrade further. On other harnesses, prefer the cheapest model that maintains accuracy on multi-rule frontmatter rewrites.

### Step 5: Aggregate Results + Deterministic Post-processing

After all sub-agents complete:

1. Aggregate results from each sub-agent:
   - success: list of successfully updated files
   - skipped: list of skipped files
   - failed: list of failed files

2. **Entity ID stripping (deterministic post-processing)**: Run `rill strip-entity-tags <file_paths ...>` in a shell on success files (ADR-046 D46-2). Safety net for cases where the LLM leaves entity IDs in tags
