---
name: newsletter
description: Generate today's daily news report at reports/newsletter/YYYY-MM-DD.md by performing WebSearch + WebFetch over the user's 3-layer context (Identity / Attention / Impulse) and producing Alert + Deep Dive + Discovery sections. Fully automated. Use when the user asks for the daily research newsletter.
gui:
  label: "/newsletter"
...
1. Search with WebSearch (each keyword independent, execute in parallel where possible)
