...
2. Read `plugins/.state/twitter.synced` to exclude files that have already been synced (treat as an empty list if the file does not exist). State lives under `plugins/.state/` so it survives `rill update` overwriting plugin directories
